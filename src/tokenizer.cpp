...
struct Tokenizer::Impl {
    ...
    explicit Impl(const std::string& spm_dir) {
        ...
        // Load vocab.json — this is the authoritative piece->id mapping,
        // identical to what AutoTokenizer uses. SPM piece strings are correct
    ...
    std::vector<int32_t> encode(const std::string& text) const {
        ...
        // Append EOS token — AutoTokenizer always does this for MarianMT
        ids.push_back(0);
        return ids;
    }
    ...
    std::string decode(const std::vector<int32_t>& ids) const {
        ...
        for (auto id : ids) {
            if (id == 0 || id == 59513) continue; // skip EOS, PAD
